...
class SSELoadTester {
    ...
    async start() {
        console.log(`Starting SSE load test with ${this.connections} connections for ${this.duration} seconds`);
    ...
    createConnection(id) {
        ...
        eventSource.onerror = (error) => {
            ...
            if (connectionInfo.connected) {
                ...
                console.log(`Connection ${id}: Reconnecting (attempt #${connectionInfo.reconnectCount})`);
            } else {
        ...
        eventSource.addEventListener('connected', (event) => {
            try {
                const data = JSON.parse(event.data);
                console.log(`Connection ${id}: Connected with ID ${data.connection_id}`);
            } catch (error) {
                console.log(`Connection ${id}: Connected (could not parse connection data)`);
            }
        });
    ...
    printStats() {
        ...
        const heartbeatsPerSecond = Math.round(this.stats.heartbeatsReceived / elapsed);
        
        console.log(`\n[${new Date().toLocaleTimeString()}] Stats (${elapsed.toFixed(1)}s elapsed):`);
        console.log(`  Active Connections: ${activeConnections}/${this.connections}`);
        console.log(`  Messages Received: ${this.stats.messagesReceived} (${messagesPerSecond}/s)`);
        console.log(`  Heartbeats: ${this.stats.heartbeatsReceived} (${heartbeatsPerSecond}/s)`);
        ...
        if (unhealthyConnections.length > 0) {
            console.log(`  ⚠️  Unhealthy connections: ${unhealthyConnections.length}`);
        }
    }
    
    printFinalReport() {
        const duration = (this.stats.endTime - this.stats.startTime) / 1000;
        const successRate = (this.stats.connectionsEstablished / 
                            (this.stats.connectionsEstablished + this.stats.connectionsFailed)) * 100;
        
        console.log(`Duration: ${duration.toFixed(1)} seconds`);
        console.log(`Target Connections: ${this.connections}`);
        console.log(`Connections Established: ${this.stats.connectionsEstablished}`);
        ...
        console.log(`Messages per Second (avg): ${Math.round(this.stats.messagesReceived / duration)}`);
        console.log(`Heartbeats per Second (avg): ${Math.round(this.stats.heartbeatsReceived / duration)}`);
    ...
    analyzeConnections() {
        ...
        for (const [id, info] of this.activeConnections) {
            ...
            console.log(`  Connection ${id}: ${info.messagesReceived} msgs (${messageRate}/s), ` +
        ...
        const avgHeartbeats = Math.round(totalHeartbeats / this.activeConnections.size);
        const avgReconnects = Math.round(totalReconnects / this.activeConnections.size * 100) / 100;
        
        console.log(`  Average per connection: ${avgMessages} msgs, ${avgHeartbeats} heartbeats, ${avgReconnects} reconnects`);
    }
    
    assessPerformance(duration, successRate) {
        const expectedHeartbeats = Math.floor(duration / 30) * this.stats.connectionsEstablished; // 30s interval
        const heartbeatDeliveryRate = (this.stats.heartbeatsReceived / expectedHeartbeats) * 100;
        
        console.log(`Expected heartbeats: ~${expectedHeartbeats}`);
        console.log(`Heartbeat delivery rate: ${heartbeatDeliveryRate.toFixed(1)}%`);
        
        // Performance criteria
        const criteria = {
            connectionSuccess: successRate >= 95,
            heartbeatReliability: heartbeatDeliveryRate >= 95,
            lowReconnectionRate: (this.stats.reconnections / this.stats.connectionsEstablished) < 0.1,
            lowErrorRate: (this.stats.errors / this.stats.messagesReceived) < 0.01
        };
        
        console.log('\nPerformance Criteria:');
        console.log(`✓ Connection Success Rate >= 95%: ${criteria.connectionSuccess ? 'PASS' : 'FAIL'} (${successRate.toFixed(1)}%)`);
        console.log(`✓ Heartbeat Delivery Rate >= 95%: ${criteria.heartbeatReliability ? 'PASS' : 'FAIL'} (${heartbeatDeliveryRate.toFixed(1)}%)`);
        console.log(`✓ Low Reconnection Rate < 10%: ${criteria.lowReconnectionRate ? 'PASS' : 'FAIL'}`);
        console.log(`✓ Low Error Rate < 1%: ${criteria.lowErrorRate ? 'PASS' : 'FAIL'}`);
    ...
    checkConnectionHealth() {
        ...
        for (const [id, info] of this.activeConnections) {
            ...
            } else if (info.lastMessage && (now - info.lastMessage) > 90000) { // 90s timeout
