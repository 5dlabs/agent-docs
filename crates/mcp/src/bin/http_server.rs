...
use anyhow::Result;
use doc_server_database::{DatabaseMigrationManager, MigrationInfo, DatabasePool, QueryPerformanceMonitor};
...
async fn main() -> Result<()> {
    ...
    if !validation_report.is_valid {
        warn!("Schema validation found {} issues", validation_report.issues.len());
    ...
    match QueryPerformanceMonitor::benchmark_queries(db_pool.pool()).await {
        Ok(results) => {
            let slow_queries: Vec<_> = results.iter()
            ...
            if slow_queries.is_empty() {
                ...
                for slow_query in &slow_queries {
                    warn!("Query '{}' exceeded 2s threshold: {}ms", 
                         slow_query.query_name, slow_query.execution_time_ms);
                }
                warn!("Found {} queries exceeding 2s performance threshold", slow_queries.len());
            }
        },
        Err(e) => {
...
fn register_core_migrations(migration_manager: &mut DatabaseMigrationManager) {
    ...
    migration_manager.register_migration(MigrationInfo {
        ...
        down_sql: Some(r#"DROP EXTENSION IF EXISTS vector; DROP EXTENSION IF EXISTS "uuid-ossp";"#.to_string()),
        dependencies: vec![],
    ...
    migration_manager.register_migration(MigrationInfo {
        ...
        version: "1.0.0".to_string(),
        description: "Create documents table for storing documentation content and embeddings".to_string(),
    ...
    migration_manager.register_migration(MigrationInfo {
        ...
        version: "1.0.0".to_string(),
        description: "Create document_sources table for source configuration management".to_string(),
    ...
    migration_manager.register_migration(MigrationInfo {
        ...
        version: "1.0.0".to_string(),
        description: "Create performance indexes for documents and document_sources tables".to_string(),
    ...
    migration_manager.register_migration(MigrationInfo {
        ...
        description: "Add foreign key constraints between documents and document_sources".to_string(),
        up_sql: fk_sql.to_string(),
        down_sql: Some("ALTER TABLE documents DROP CONSTRAINT IF EXISTS fk_documents_source;".to_string()),
        dependencies: vec!["003_documents_table".to_string(), "004_document_sources_table".to_string()],
    ...
    migration_manager.register_migration(MigrationInfo {
        ...
        description: "Add monthly range partitioning to documents table for performance and archival".to_string(),
    ...
    migration_manager.register_migration(MigrationInfo {
        ...
        version: "1.2.0".to_string(),
        description: "Create archival system for old documents (>1 year) with automated procedures".to_string(),
