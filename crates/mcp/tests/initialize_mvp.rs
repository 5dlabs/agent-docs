...
async fn test_initialize_protocol_version_2025_06_18() {
    ...
    match DatabasePool::new(&database_url).await {
        Ok(db_pool) => {
            let handler = McpHandler::new(db_pool).await.expect("Failed to create handler");
            ...
            let capabilities = response.get("capabilities").expect("Should have capabilities");
            assert!(
        ...
        Err(_) => {
            ...
            assert_eq!(
                expected_response.get("protocolVersion").and_then(|v| v.as_str()),
                Some("2025-06-18")
            );
