...
fn parse_source_path(s: &str) -> Result<(DocType, PathBuf), String> {
    ...
    if parts.len() != 2 {
        return Err("Source path must be in format 'type=path'".to_string());
    }

    let doc_type = match parts[0].to_lowercase().as_str() {
        "rust" => DocType::Rust,
        ...
        _ => return Err(format!("Unknown document type: {}", parts[0])),
    };

    Ok((doc_type, PathBuf::from(parts[1])))
}
...
async fn main() -> Result<()> {
    ...
    match args.command {
        ...
        MigrateCommand::Rollback { batch_id } => {
            ...
            match pipeline.rollback_batch(batch_uuid).await {
                Ok(_) => {
