...
mod tests {
    ...
    async fn test_openai_client_creation() -> Result<()> {
        ...
        info!("Successfully generated individual embedding with {} dimensions", embedding.len());
        ...
        if let Some(submitted_batch_id) = processor.flush_current_batch().await? {
            ...
            if let Some(batch) = processor.get_batch(&submitted_batch_id).await {
                ...
                if let Some(cost_info) = &stats.cost_info {
                    ...
                    info!("  Savings: ${:.6} ({})", cost_info.savings_usd, cost_info.savings_percentage_formatted());
                    
                    // Verify cost savings
                    assert!(cost_info.savings_percentage > 40.0, "Expected at least 40% cost savings");
                }
                ...
                for result in &batch.results {
                    if result.error.is_none() {
                        assert_eq!(result.embedding.len(), 3072, "Embedding should have 3072 dimensions");
                        assert!(!result.embedding.iter().all(|&x| x == 0.0), "Embedding should not be all zeros");
                    }
                }
            }
        }
    ...
    async fn test_rate_limiting() -> Result<()> {
        ...
        info!("Successfully completed {} out of 5 rate-limited requests", successful_requests);
        assert!(successful_requests >= 3, "At least 3 requests should succeed with rate limiting");
    ...
    async fn test_cost_calculation_accuracy() -> Result<()> {
        ...
        info!("  Savings percentage: {}", cost_info.savings_percentage_formatted());
    ...
    async fn test_batch_processor_cleanup() -> Result<()> {
        ...
        let removed_count = processor.cleanup_old_batches(Duration::from_secs(1)).await?;
    ...
    async fn test_performance_benchmarks() -> Result<()> {
        ...
        info!("Added 1000 requests in {:?} ({:.2} req/sec)", 
              add_duration, 
              1000.0 / add_duration.as_secs_f64());
        
        // Verify performance expectations
        assert!(add_duration < Duration::from_secs(5), "Adding 1000 requests should take less than 5 seconds");
