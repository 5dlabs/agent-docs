...
impl Tool for AddRustCrateTool {
    ...
    async fn execute(&self, arguments: Value) -> Result<String> {
        ...
        // Enqueue the job - this will create a persistent job record
        ...
        Ok(format!(
            "✅ **Crate ingestion job enqueued successfully**\n\n\
            **Job ID:** `{}`\n\
            **Crate Name:** {}\n\
            **Version:** {}\n\
            **Status:** Queued → Running\n\n\
            🔄 The crate is being processed in the background. Use `check_rust_status` with the job_id to monitor progress and get completion status.",
            job_id,
            crate_name,
